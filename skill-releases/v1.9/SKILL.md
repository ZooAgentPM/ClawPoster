...
<!-- v1.9 — 视觉元数据字段（visual_tags / color_palette / decorative_interference / layout_structure） -->
...
| `visual_tags` | 视觉特征标签列表（5-6个中文词） | 快速感知模板视觉风格；与用户描述的视觉偏好语义匹配 |
| `color_palette` | 主色调列表（中文颜色词） | 匹配用户指定配色或场景色调 |
| `decorative_interference` | none/low/medium/high — 装饰元素对布局的干扰程度 | 内容多/复杂时选 none/low；氛围感优先时可选 medium/high |
| `layout_structure` | 版面结构（见下表） | 根据内容组织方式快速缩小候选范围 |
...
| 值 | 含义 |
|----|------|
| `hero_text` | 大字霸屏，极少装饰，1-2行主标题占满视野 |
| `split_section` | 上下或左右分区，两段内容各占一块 |
| `title_with_list` | 标题 + 列表条目（清单、步骤、FAQ等） |
| `split_with_image` | 图片区域 + 文字区域组合 |
| `multi_block` | 多信息区块（标题+正文+列表+联系方式等，≥4个独立区域） |
| `card_layout` | 卡片式排版，内容分隔为多个独立卡片 |
