...
| 直接 curl port 7002 创建状态页 | 必须走 port 3001，否则 task_id 绑定失败 |
...
任务开始时立刻给用户一个可随时查看的状态页链接，每步执行后实时更新，消除等待黑盒感。
...
**第三步**：创建播报 cron，保存 CRON_ID（`to`、`account_id`、`channel` 从 deliveryContext 获取）：

```bash
CRON_ID=$(bash {skill_dir}/scripts/start_notify.sh \
  "{deliveryContext.to}" "{deliveryContext.account_id}" \
  "$PROGRESS_FILE" "{deliveryContext.channel}" "30s")
```

⚠ 此步骤**不可跳过**：cron 只读文件并转发内容，不做任何清理操作，安全无副作用。

### 执行过程

**无需手动上报。** MCP 工具（ensure_services / search_templates / get_template_spec / generate_poster / get_slot_crops）执行完后会**自动**向状态页推送进度，不需要额外 curl。
...
**负向风格排除**（可选）：当结果风格不对时，用 `exclude` 参数用向量减法推离不想要的风格方向。
```
search_templates(query="...", exclude="古风 国风 水墨 手绘", usage_type="cover", n=8)
```
`exclude` 填风格关键词即可，不需要完整句子。注意：这是向量空间的方向排斥，不是关键词过滤。

无结果时改用 `list_templates(keyword)` 兜底。
...
### Step 3.5：get_template_spec(template_id)

⚠ `template_id` 纯整数，不带 `t` 前缀。
...
**槽位内容：** 读取 `references/` 应用文案方法论。

### Step 5：generate_poster(template_id, slots, adjustments?)

工具返回后上报状态页（data 带 render_url）。
...
3张以上时，**主 Agent 持有唯一 cron**，子 Agent 只负责生图并回传结果。

### 主 Agent 职责

1. 创建进度文件（mode 写 batch，total 写总张数）：
...
3. 并行启动 N 个子 Agent，**等待所有子 Agent 回传**
4. 每收到一个子 Agent 的结果，立即更新进度文件（done +1，version +1，正文追加该张链接）
5. 全部完成后写 status=done
...
子 Agent **不创建 cron、不写进度文件**，只做完整生图流程（Step 1~6），完成后回传：
...
主 Agent 收到回传后负责把 render_url 写入进度文件。
...
| 陷阱 | 表现 | 解决 |
|------|------|------|
| 状态页走了 port 7002 | task_id 无法绑定 session，状态页空白 | 必须 POST port 3001/status/create |
...
| render URL 用本地地址 | 用户无法访问 | 通过 cron 发公网 URL，不自行构造路径 |
