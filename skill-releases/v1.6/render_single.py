...
def render_single(
    ...
    with sync_playwright() as pw:
        ...
        # 注入槽位文本
        # 优先通过 Vue reactive（pageGroup ref）注入，避免直接 DOM 操作触发
        # Chromium GPU 合成层顺序错误（textEffects 模板的描边层盖住填充层导致颜色变黑）。
        # 无法从 pageGroup 找到 widget 时回退为 DOM 注入（保留 SVG 装饰 + z-index 修正）。
        if slots:
            ...
            result = page.evaluate(f"""() => {{
                ...
                // 遍历 Vue vnode 树，找到含 pageGroup 的 Html.vue 实例
                ...
                for (const [uuid, text] of Object.entries(slots)) {{
                    // 优先：Vue reactive 注入（不触发 GPU 合成层问题）
                    ...
                    // 回退：DOM 注入（无 textEffects 的普通文本层）
                    ...
                    targets.forEach(div => {{
                        ...
                        if (isEffect) {{
                            ...
                            // 保留 rough-annotation SVG，用 span + z-index 压住 SVG
